...
for i in range(50000):
    ...
    cartsToRemove = []
    toTestCarts = carts.copy()
    toTestCarts = sorted(toTestCarts, key=lambda g: g["position"][0] * 1000 + g["position"][1])
    for cart in toTestCarts:
        ...
        for cart in carts:
            if cart["position"] in positions:
                index = positions.index(cart["position"])
                print(len(carts))
                oldcart = carts[index]
    ...
    if len(carts) == 1:
        print("part 2 answer is: " + str(carts[0]["position"][1]) + "," + str(carts[0]["position"][0]))
        exit()
